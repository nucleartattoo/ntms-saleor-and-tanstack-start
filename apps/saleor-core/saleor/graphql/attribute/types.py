...
class Attribute(ChannelContextType[models.Attribute]):
    ...
    filterable_in_storefront = graphene.Boolean(
        description="Whether the attribute can be filtered in storefront (compatibility field).",
        required=False,
    )
    available_in_grid = graphene.Boolean(
        description="Whether the attribute is available in grid (compatibility field).",
        required=False,
    )
    storefront_search_position = graphene.Int(
        description="The position of the attribute in the storefront search (compatibility field).",
        required=False,
    )
    translation = TranslationField(
    ...
    def resolve_filterable_in_storefront(
        ...
        return getattr(root.node, "filterable_in_storefront", False)

    @staticmethod
    def resolve_available_in_grid(
        root: ChannelContext[models.Attribute], _info: ResolveInfo
    ):
        return getattr(root.node, "available_in_grid", False)

    @staticmethod
    def resolve_storefront_search_position(
        root: ChannelContext[models.Attribute], _info: ResolveInfo
    ):
        return getattr(root.node, "storefront_search_position", 0)
