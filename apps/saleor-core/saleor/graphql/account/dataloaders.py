...
class AccessibleChannelsByUserIdLoader(BaseAccessibleChannels[str]):
    ...
    def batch_load(self, keys):
        superusers = set(
            User.objects.using(self.database_connection_name)
            .filter(id__in=keys, is_superuser=True)
            .values_list("id", flat=True)
        )
        all_channels = None
        if superusers:
            all_channels = set(
                Channel.objects.using(self.database_connection_name).all()
            )
        ...
        for su_id in superusers:
            user_to_channels[su_id] = all_channels
...
class RestrictedChannelAccessByUserIdLoader(DataLoader[int, bool]):
    ...
    def batch_load(self, keys):
        ...
        for su_id in superusers:
            user_to_restricted_access[su_id] = False
